...
func _ready():
	...
	set_char_direction(input_direction)
	
func _physics_process(delta):
	if player_state == PlayerState.TURNING or stop_input == true:
		return
...
#--- Direction ---
func set_char_direction(direction: Vector2):
...
func move(delta):
	...
	if door_ray.is_colliding():
		...
		if percent_moved_to_next_tile >= 2.0:
			...
			position.y = initial_position.y + (-0.16 - 0.23 * input + 0.05 * pow(input, 2))

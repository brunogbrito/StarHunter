...
func _ready():
	player = find_parent("CurrentScene").get_children().back().find_node("Player")
...
func player_in_grass():
	if player_inside == true:
		...
		grass_step_effect.position = position + Vector2(16, 0)
		...
		grass_overlay.rect_position = position + Vector2(16, 0)
